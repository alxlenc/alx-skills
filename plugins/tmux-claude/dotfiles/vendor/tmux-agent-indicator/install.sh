...
install_statusline = False  # alx-skills local patch (see VENDORED.md): hooks only, never wrap statusLine
if install_statusline and mode == "install" and (status_line is None or isinstance(status_line, dict)):
    if not wrapped:
